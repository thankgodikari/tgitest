    ...
    constructor() {
        ...
        this.config = {
            ...
            normal: {
                ...
                bearishMultiplier: 1.87,         // Target when trend is weak
            ...
            recovery: {
                ...
                blockConsecutiveLowCrashes: 4,                  // block when X many consecutive low crashes
                consecutiveLookback: 8,                         // lookback used by consecutive logic
                ...
                maxRiskPct: 0.9,                                // never risk more than 90% of balance on a recovery bet
...
class BettingGateController {
    ...
    onPartialRecoveryWin({remainingDebtBits} = {}) {
        try {
            // Deny the immediate next round (1 deny) after a partial recovery win,
            // and close the gate to ensure no accidental placement until ensemble approves.
            this._denyRounds = Math.max(this._denyRounds || 0, 1);

            // prevent followups from re-opening the gate after the deny rounds expire —
            // only an EV request may open the gate (and that EV request will clear this flag).
            this._postPartialRecoveryLock = true;

            if (typeof this.closeGate === 'function') this.closeGate('PARTIAL-RECOVERY-WIN');
            try {
                if (this.logger && typeof this.logger.log === 'function') this.logger.log(`🔒 Betting gate: denying next round after partial recovery win (remaining debt: ${Number(remainingDebtBits || 0)} bits)`, 'info');
            } catch (e) {
            }
        } catch (e) {
            try {
...
// ============================
// FollowUpScheduler (tiny)
// Responsible only for scheduling follow-up slots immediately after an EV-scheduled bet loses.
// It does NOT place bets itself — it just returns whether a follow-up is scheduled and how many slots remain.
// ============================
class FollowUpScheduler {
    constructor(bot) {
        this.bot = bot;
        this.current = null; // { batchId, remaining, total, isForced }
        this.bot = (bot && bot.config) ? bot : ((bot && typeof bot === 'object') ? {config: bot} : null);
    }

    // extended: allow scheduling with optional inter-slot wait (waitBetweenRounds)
    // waitBetweenRounds: integer >= 0 specifying how many rounds to wait between attempts.
    scheduleFollowups({batchId = null, count = 0, isForced = false, waitBetweenRounds = 0} = {}) {
        if (!this.bot || !this.bot.config) return false;

        const followCfg = (this.bot.config && this.bot.config.recovery && this.bot.config.recovery.followUp) ? this.bot.config.recovery.followUp : null;

        if (!followCfg || !followCfg.enabled) {
            return false;
        }

        const maxAllowed = Number.isFinite(Number(followCfg.maxCount)) ? Number(followCfg.maxCount) : 0;
        const actual = Math.min(Number(count), maxAllowed);
        if (actual <= 0) return false;

        // nextAvailableRound is measured in bot.rounds (round counter). Start from current bot.rounds.
        const startRound = Number((this.bot && Number.isFinite(this.bot.rounds)) ? this.bot.rounds : 0);
        this.current = {
            batchId: batchId || (`fu-${Date.now()}`),
            remaining: actual,
            total: actual,
            isForced: !!isForced,
            waitBetweenRounds: Math.max(0, Math.floor(Number(waitBetweenRounds) || 0)),
            nextAvailableRound: startRound + Math.max(0, Math.floor(Number(waitBetweenRounds) || 0))
        };

        try {
            if (this.bot && this.bot.logger) this.bot.logger.log(`Follow-up scheduled: ${actual} slots (batch=${this.current.batchId}) waitBetweenRounds=${this.current.waitBetweenRounds}`, 'info');
        } catch (e) {
        }
    ...
    getNextFollowup() {
        if (!this.current || this.current.remaining <= 0) return null;

        // If nextAvailableRound is set and bot.rounds hasn't reached it yet, do not return a slot.
        const botRounds = Number((this.bot && Number.isFinite(this.bot.rounds)) ? this.bot.rounds : 0);
        if (Number.isFinite(Number(this.current.nextAvailableRound)) && botRounds < Number(this.current.nextAvailableRound)) {
            // not yet time to try the next follow-up slot
            return null;
        }

        // it's time: consume one slot and schedule next availability
        this.current.remaining = Math.max(0, this.current.remaining - 1);
        const slot = {batchId: this.current.batchId, remaining: this.current.remaining, total: this.current.total};

        // schedule next time (if remaining) using waitBetweenRounds
        if (this.current.remaining > 0 && Number.isFinite(Number(this.current.waitBetweenRounds))) {
            this.current.nextAvailableRound = botRounds + Number(this.current.waitBetweenRounds);
        } else {
            // all slots consumed
            this.current = null;
        }
...
class BettingEngine {
    constructor(config, logger) {
        ...
        // --- Proxy so BettingEngine can safely call _updateFollowupResultTracker(...) ---
        // BettingEngine.placeBet() calls `this._updateFollowupResultTracker(...)` in several places.
        // Provide a local proxy that forwards to the bot's CrashBot._updateFollowupResultTracker when available.
        // This avoids "undefined function" errors when `this` is the BettingEngine instance.
        this._updateFollowupResultTracker = (batchId, stakeBits) => {
            try {
                if (this.bot && typeof this.bot._updateFollowupResultTracker === 'function') {
                    return this.bot._updateFollowupResultTracker(batchId, stakeBits);
                }
            } catch (e) {
                // Non-fatal: log proxy forwarding issues but do not throw.
                try {
                    if (this.logger) this.logger.log(`⚠️ BettingEngine._updateFollowupResultTracker proxy error: ${e && e.message ? e.message : String(e)}`, 'warning');
                } catch (e2) {
                }
            }
        };
    ...
    placeBet(amount, targetMultiplier, modeOrMeta = undefined) {
        ...
        const reconcileAsPlaced = (placedAmount, placedTargetMultiplier, normalizedBatchId) => {
            ...
            // ONLY call follow-up tracker after we have confirmed the bet
            try {
                this._updateFollowupResultTracker(this.activeBet.batchId, this.activeBet.stakeBits);
            } catch (e) {
            }
...
class CrashBot {
    constructor() {
        ...
        try {
            this.followUpScheduler = this.followUpScheduler || new FollowUpScheduler(this);
        } catch (e) { /* defensive: scheduler may be injected at runtime */
        }
        ...
        try {
            this.followUpScheduler = new FollowUpScheduler(this);
        } catch (e) {
    ...
    _updateFollowupResultTracker(batchId, stakeBits) {
        // tiny compatibility adapter so other code that expects followupResultTracker keeps working
        try {
            this.followupResultTracker = this.followupResultTracker || {
                lastBatchId: null,
                lastBetStakeBits: 0,
                lastFollowIndex: 0
            };
            this.followupResultTracker.lastBatchId = (typeof batchId !== 'undefined' && batchId !== null) ? batchId : null;
            this.followupResultTracker.lastBetStakeBits = Number(stakeBits || 0);

            // if FollowUpScheduler has a current batch, keep lastFollowIndex roughly in sync
            if (this.followUpScheduler && this.followUpScheduler.current) {
                const total = Number(this.followUpScheduler.current.total || 0);
                const remaining = Number(this.followUpScheduler.current.remaining || 0);
                this.followupResultTracker.lastFollowIndex = Math.max(0, total - remaining);
            }
        } catch (e) {
            try {
                if (this.logger) this.logger.log(`⚠️ _updateFollowupResultTracker adapter error: ${e && e.message}`, 'warning');
            } catch (ee) {
            }
        }
    }
        ...
        try {
            ...
            engine.on('game_starting', (info) => {
                ...
                if (String(this.mode || '').toUpperCase() === 'WARMUP') {
                    ...
                    try {
                        if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                            // allow scheduler clear (non-authoritative flows), but do NOT drop an active authoritative follow-up lock
                            try {
                                // if an authoritative follow-up is active, we prefer to preserve it; let authoritative flow exhaust itself
                                if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                    this.followUpScheduler.clear();
                                } else {
                                    try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve followUpScheduler due to active authoritative follow-ups (warmup)', 'debug'); } catch(_) {}
                                }
                            } catch (_) { /* swallow non-fatal */ }
                        }
                    } catch (e) { }
                    try {
                        // Do not clear an active authoritative follow-up schedule (let authoritative flow exhaust it).
                        if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                            this.pendingFollowUpsSchedule = null;
                        } else {
                            try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve pendingFollowUpsSchedule due to active authoritative follow-ups (warmup)', 'debug'); } catch(_) {}
                        }
                    } catch (e) {}
                    try {
                ...
                // 1) Scheduled FOLLOW-UP (highest priority)
                try {
                    const followCurrent = (this.followUpScheduler && this.followUpScheduler.current) ? this.followUpScheduler.current : null;

                    // Enforce strict gating: only allow scheduled follow-ups that were explicitly created
                    // by an EV-triggered recovery loss (pendingFollowUpsSchedule must exist and match).
                    if (followCurrent && Number(followCurrent.remaining || 0) > 0 && !this.betting.hasActiveBet()) {

                        // additional verification: pendingFollowUpsSchedule must exist and match the batchId
                        // If pendingFollowUpsSchedule is missing or mismatched, attempt a safe reconstruction
                        if (!this.pendingFollowUpsSchedule || String(this.pendingFollowUpsSchedule.batchId || '') !== String(followCurrent.batchId || '')) {
                            try {
                                // If the scheduler has the authoritative current entry and batchId matches, rebuild the pending marker.
                                if (this.followUpScheduler && this.followUpScheduler.current && String(this.followUpScheduler.current.batchId || '') === String(followCurrent.batchId || '')) {
                                    // Reconstruct minimal pendingFollowUpsSchedule (use existing scheduler fields)
                                    this.pendingFollowUpsSchedule = {
                                        batchId: String(this.followUpScheduler.current.batchId || followCurrent.batchId || `followup-${Date.now()}`),
                                        createdAt: Date.now(),
                                        slots: Number.isFinite(Number(this.followUpScheduler.current.remaining)) ? Number(this.followUpScheduler.current.remaining) : Number(this.followUpScheduler.current.total || 0),
                                        total: Number.isFinite(Number(this.followUpScheduler.current.total)) ? Number(this.followUpScheduler.current.total) : Number(this.followUpScheduler.current.remaining || 0),
                                        waitBetweenRounds: Number.isFinite(Number(this.followUpScheduler.current.waitBetweenRounds)) ? Number(this.followUpScheduler.current.waitBetweenRounds) : 0,
                                        isForced: !!this.followUpScheduler.current.isForced,
                                        triggeringEvBatchId: (this._lastEvRecoveryBatch ? String(this._lastEvRecoveryBatch) : null)
                                    };
                                    try {
                                        this.logger.log('⚠️ Reconstructed missing pendingFollowUpsSchedule from followUpScheduler.current — resuming follow-up placement', 'info');
                                    } catch (_) {
                                    }
                                } else {
                                    // Cannot safely reconstruct — skip placement (the original safety guard)
                                    try {
                                        this.logger.log('⚠️ Scheduled follow-up present but pendingFollowUpsSchedule missing/mismatch — skipping follow-up placement', 'warning');
                                    } catch (_) {
                                    }
                                    try {
                                        this.updatePanel(this.betting.hasActiveBet(), 'RECOVERY');
                                    } catch (e) {
                                    }
                                    return;
                                }
                            } catch (e) {
                                // Defensive fallback: do not crash the consumer; preserve previous behaviour.
                                try {
                                    this.logger.log(`⚠️ scheduled follow-up reconstruction failed: ${e && e.message}`, 'warning');
                                } catch (_) {
                                }
                                try {
                                    this.updatePanel(this.betting.hasActiveBet(), 'RECOVERY');
                        ...
                        // Ensure we still have slots to attempt (never exceed configured maxCount)
                        if (!Number.isFinite(Number(this.pendingFollowUpsSchedule.slots)) || Number(this.pendingFollowUpsSchedule.slots) <= 0) {
                            try {
                                this.logger.log('🔒 Pending follow-up slots exhausted — clearing follow-up scheduler', 'info');
                            } catch (_) {
                            }
                            try {
                                if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                                    // Only clear scheduler if no active authoritative follow-ups remain
                                    try {
                                        if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                            this.followUpScheduler.clear();
                                        } else {
                                            try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve followUpScheduler due to active authoritative follow-ups (exhausted branch)', 'debug'); } catch(_) {}
                                        }
                                    } catch (_) { /* swallow */ }
                                }
                            } catch (e) {}
                            try {
                                if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                    ...
                                    try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve pendingFollowUpsSchedule due to active authoritative follow-ups (exhausted branch)', 'debug'); } catch(_) {}
                                }
                            } catch (e) {}
                            try {
                                if (this.bettingGate && typeof this.bettingGate.closeGate === 'function') this.bettingGate.closeGate('FOLLOWUP-EXHAUSTED');
                                // Impose a pause after the follow-up sequence is exhausted
                                // We use postRecoveryPauseRounds (default 2) to force a wait.
                                const postPause = (this.config && this.config.normal && Number.isFinite(Number(this.config.normal.postRecoveryPauseRounds)))
                                    ? Number(this.config.normal.postRecoveryPauseRounds)
                                    : 2; // Fallback to 2 rounds

                                if (this.bettingGate) {
                                    // Set denyRounds to enforce the pause
                                    this.bettingGate._denyRounds = Math.max(this.bettingGate._denyRounds || 0, postPause);
                                    if (typeof this.bettingGate.closeGate === 'function') {
                                        this.bettingGate.closeGate('FOLLOWUP-EXHAUSTED-PAUSE');
                                    }
                                    this.logger.log(`ℹ️ Post-follow-up pause: denying bets for ${postPause} rounds.`, 'info');
                                }
                            } catch (e) {
                            }
                            return;
                        ...
                        // ========================================================================
                        // STRICT SNIPER LOGIC FOR FOLLOW-UPS
                        // ========================================================================
                        // 1. Prepare context for the Sniper Check
                        const projectedStakeSat = (typeof this.resolveCanonicalStakeSat === 'function')
                            ? Number(this.resolveCanonicalStakeSat(null, null, 'RECOVERY')) : 0;
                        const balanceSat = (typeof this.getCurrentBalance === 'function') ? Number(this.getCurrentBalance()) : 0;
                        const targetMultiplier = Number((this.config && this.config.recovery && this.config.recovery.multiplier) || 2.0);

                        const ctx = {
                            EV: (this.ev && typeof this.ev.expectedValue === 'function') ? this.ev.expectedValue(targetMultiplier, Math.max(1, projectedStakeSat)) : null,
                            ensemble: {prob: 0.5}, // Dummy value, Sniper check is primary
                            balanceSat: Number(balanceSat || 0),
                            history: this.stats,
                            forceBypass: false
                        };

                        // 2. Ask RecoveryManager: Is it safe to bet?
                        // We assume safe unless the manager explicitly says NO.
                        let gateAllowed = false;

                        if (this.recoveryManager && typeof this.recoveryManager.canPlaceRecoveryBet === 'function') {
                            // Pass isFollowUp = true
                            const isSafe = this.recoveryManager.canPlaceRecoveryBet(ctx, true, 0);

                            if (!isSafe) {
                                try {
                                    this.logger.log('🛑 RecoveryManager (Sniper) PAUSED scheduled follow-up. Market unsafe (< Threshold).', 'warning');
                                } catch (e) {}
                                try {
                                    this.updatePanel(this.betting.hasActiveBet(), 'RECOVERY-PAUSED-UNSAFE');
                                } catch (e) {}

                                // IMPORTANT: We return here WITHOUT placing the bet.
                                // We do NOT clear the scheduler. The slot remains "Pending" and will retry next round.
                                return;
                            } else {
                                // 3. SUCCESS: Market is safe. Open the gate.
                                gateAllowed = true;
                            }
                        } else {
                            // Fallback if manager missing (should not happen): Allow it to prevent deadlock
                            gateAllowed = true;
                        }

                        // 4. Final Gate Check (redundant safety, but keeps flow consistent)
                        if (!gateAllowed) {
                            try { this.logger.log('⏭️ Betting gate denied follow-up.', 'skip'); } catch (_) {}
                            return;
                        }
                        // ========================================================================

                        // --- Authoritative follow-up consumption (highest priority) ---
                        // If we have an authoritative follow-up lock, consume from it (bypass scheduler gating).
                        let slot = null;
                        try {
                            if (this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0) {
                                // ensure we are still within expire window
                                const botRoundsNow = Number((this.bot && Number.isFinite(this.bot.rounds)) ? this.bot.rounds : (Number.isFinite(this.rounds) ? this.rounds : 0));
                                if (Number.isFinite(Number(this._authoritativeFollowup.expireRound)) && botRoundsNow > Number(this._authoritativeFollowup.expireRound)) {
                                    // expired — clear lock (fall back to scheduler)
                                    this._authoritativeFollowup.active = false;
                                } else {
                                    // consume one authoritative slot
                                    this._authoritativeFollowup.remaining = Math.max(0, Number(this._authoritativeFollowup.remaining) - 1);
                                    // keep batch id for logging & downstream meta
                                    slot = { batchId: this._authoritativeFollowup.batchId };
                                    // also decrement canonical pendingFollowUpsSchedule if present
                                    try { if (this.pendingFollowUpsSchedule && typeof this.pendingFollowUpsSchedule.slots !== 'undefined') this.pendingFollowUpsSchedule.slots = Math.max(0, Number(this.pendingFollowUpsSchedule.slots) - 1); } catch(_) {}

                                    // *** Ensure FollowUpScheduler.current also decrements when authoritative slot is consumed. ***
                                    // Without this, reconstruction from followUpScheduler.current can re-inflate remaining slots.
                                    try {
                                        if (this.followUpScheduler && this.followUpScheduler.current && typeof this.followUpScheduler.current.remaining !== 'undefined') {
                                            this.followUpScheduler.current.remaining = Math.max(0, Number(this.followUpScheduler.current.remaining) - 1);
                                            // If scheduler remaining is exhausted, clear scheduler to avoid future reconstruction restoring slots
                                            if (Number(this.followUpScheduler.current.remaining) <= 0) {
                                                try { if (typeof this.followUpScheduler.clear === 'function') this.followUpScheduler.clear(); } catch(_) {}
                                            } else {
                                                // ensure nextAvailableRound remains valid (no-op), but keep canonical state in sync
                                                // (we don't change nextAvailableRound here — getNextFollowup will manage scheduling)
                                            }
                                        }
                                    } catch(_) { /* swallow — non fatal */ }
                                }
                        ...
                        // If no authoritative slot found, fall back to scheduler.current/normal path
                        if (!slot) {
                            slot = (typeof this.followUpScheduler.getNextFollowup === 'function') ? this.followUpScheduler.getNextFollowup() : null;
                        }

                        if (!slot) {
                            try { this.logger.log('⚠️ no follow-up slot available to consume despite scheduler indicating remaining', 'warning'); } catch (_) {}
                            try { this.updatePanel(this.betting.hasActiveBet(), 'RECOVERY'); } catch (e) {}
                            return;
                        }
                        // continue as before: place the follow-up using slot.batchId, amountSat, etc.

                        // --- A: SAFE EV confirmation check (improved: block single-spike-trigger) ---
                        try {
                            // canonical pending EV request must exist
                            const evReqLocal = (typeof this.pendingEvRequest !== 'undefined' && this.pendingEvRequest) ? this.pendingEvRequest : null;
                            if (!evReqLocal || typeof evReqLocal.amountSat === 'undefined' || typeof evReqLocal.multiplier === 'undefined') {
                                this.clearPendingEvRequest && this.clearPendingEvRequest();
                                return;
                            }

                            // must have EV module expectedValue
                            ...
                            let evEstimateLocal ;
                            try {
                                evEstimateLocal = this.ev.expectedValue(Number(evReqLocal.multiplier), Number(evReqLocal.amountSat));
                            } catch (_) {
                                evEstimateLocal = null;
                            }

                            const evWindowLocal = (this.ev && Number.isFinite(Number(this.ev.window))) ? Number(this.ev.window) : null;
                            const evSamplesRaw = (evEstimateLocal && (evEstimateLocal.sampleSize || evEstimateLocal.samples || evEstimateLocal.window)) ? Number(evEstimateLocal.sampleSize || evEstimateLocal.samples || evEstimateLocal.window) : (evWindowLocal || 0);
                            const evSamplesLocal = Number.isFinite(evSamplesRaw) ? evSamplesRaw : 0;
                            const evValueLocal = (evEstimateLocal && Number.isFinite(Number(evEstimateLocal.evPerUnit))) ? Number(evEstimateLocal.evPerUnit) :
                                (evEstimateLocal && Number.isFinite(Number(evEstimateLocal.evAbsolute))) ? (Number(evEstimateLocal.evAbsolute) / Math.max(1, Number(evReqLocal.amountSat))) : 0;

                            // Check config EVsettings presence
                            const evCfgLocal = (this.config && this.config.recovery && this.config.recovery.EVsettings) ? this.config.recovery.EVsettings : null;

                            // If config requires minimum sample check, enforce it (only if the correct key exists)
                            // NOTE: the config uses `minEvSamples` (see config.recovery.minEvSamples), not recoveryMinSamples
                            if (evCfgLocal && typeof evCfgLocal.minEvSamples !== 'undefined') {
                                const minEvCfg = Number(evCfgLocal.minEvSamples);
                                if (Number.isFinite(minEvCfg) && evWindowLocal) {
                                    const minSamplesRequired = (minEvCfg < 1) ? Math.max(1, Math.ceil(minEvCfg * evWindowLocal)) : Math.max(1, Math.floor(minEvCfg));
                                    if (evSamplesLocal < minSamplesRequired) {
                                        this.clearPendingEvRequest && this.clearPendingEvRequest();
                                        return;
                                    }
                                }
                            ...
                            // LAST CRASH check: if the most recent single round crash was a HIGH CRASH (>= chosen multiplier),
                            // then require stronger confirmation (prevents single-spike-trigger).
                            // Determine which multiplier to use (recovery mode uses recovery.multiplier, otherwise normal.multiplier)
                            let modeMultiplier = null;
                            try {
                                if (String(this.mode || '').toLowerCase() === 'recovery') {
                                    modeMultiplier = Number(this.getCanonicalRecoveryMultiplier() || 0);
                                }
                            } catch (_) {
                                modeMultiplier = null;
                            }

                            // get last 1-2 crashes via existing api
                            let lastCrashes = [];
                            try {
                                if (this.stats && typeof this.stats.getRecentCrashes === 'function') {
                                    lastCrashes = this.stats.getRecentCrashes(2) || [];
                                }
                            } catch (_) {
                                lastCrashes = [];
                            }

                            // if last round exists and is a spike >= modeMultiplier, treat it specially
                            const lastCrashObj = (lastCrashes && lastCrashes.length > 0) ? lastCrashes[0] : null;
                            const lastCrashValue = (lastCrashObj && typeof lastCrashObj.crash !== 'undefined') ? Number(lastCrashObj.crash) : null;

                            if (modeMultiplier !== null && lastCrashValue !== null && lastCrashValue >= Number(modeMultiplier)) {
                                // A high crash was observed last round. Require consecutive confirmation.
                                // Determine required confirmation count from config, default to 2 only when the config key is present.
                                const requiredConfirm = (evCfgLocal && typeof evCfgLocal.recoveryConsecutiveConfirm !== 'undefined') ? Number(evCfgLocal.recoveryConsecutiveConfirm) : 0;
                                const confirmWindow = (evCfgLocal && typeof evCfgLocal.recoveryConfirmWindow !== 'undefined') ? Number(evCfgLocal.recoveryConfirmWindow) : 0;
                                if (!requiredConfirm || !confirmWindow) {
                                    // config didn't provide confirmation settings — be conservative: cancel EV scheduling
                                    this.clearPendingEvRequest && this.clearPendingEvRequest();
                                    return;
                                }

                                // compute recent confirmation flags using existing stats.getRecentCrashes(confirmWindow)
                                // Defensive: if the lastCrashValue was a spike (we're in the 'special' case), fetch confirmWindow+1
                                // and drop the immediate last crash to avoid spike contamination.
                                let recent = [];
                                try {
                                    if (this.stats && typeof this.stats.getRecentCrashes === 'function') {
                                        // fetch one extra and drop the most recent (index 0) only in the 'last-spike' case earlier
                                        recent = this.stats.getRecentCrashes(Math.max(1, confirmWindow + 1)) || [];
                                        // drop immediate spike (index 0) because requiredConfirm is meant to refer to confirmations after the spike
                                        if (recent.length > 0) recent = recent.slice(1);
                                        // cap to confirmWindow if we fetched extra
                                        recent = recent.slice(0, confirmWindow);
                                    } else {
                                        recent = [];
                                    }
                                } catch (_) {
                                ...
                                const threshold = Number(evCfgLocal.recoveryEvThreshold || 0);
                                const recentFlags = recent.map(r => !!(r && (typeof r.ev !== 'undefined') ? (Number(r.ev) >= threshold) : false));
                                const confirmCnt = recentFlags.reduce((a, b) => a + (b ? 1 : 0), 0);
                                if (confirmCnt < Number(requiredConfirm)) {
                                    // Not enough confirmation following the spike — cancel scheduling
                                    this.clearPendingEvRequest && this.clearPendingEvRequest();
                                    return;
                                }
                            }

                            // Delta gating: apply only if configured
                            if (evCfgLocal && typeof evCfgLocal.recoveryEvDeltaReq !== 'undefined' && typeof this._lastEvPlacedValue !== 'undefined' && this._lastEvPlacedValue !== null) {
                                const deltaReqLocal = Number(evCfgLocal.recoveryEvDeltaReq);
                                if (Number.isFinite(Number(this._lastEvPlacedValue)) && (evValueLocal < (Number(this._lastEvPlacedValue) + deltaReqLocal))) {
                                    this.clearPendingEvRequest && this.clearPendingEvRequest();
                                    return;
                                }
                            }

                            // Passed all checks — allow downstream placement
                        } catch (err) {
                            // defensive: on unexpected error, clear pending ev request
                            try {
                                this.logger && this.logger.log && this.logger.log(`EV check error: ${err && err.message}`, 'warning');
                            } catch (_) {
                            }
                            this.clearPendingEvRequest && this.clearPendingEvRequest();
                            return;
                        }
                        // --- end SAFE EV confirmation check (improved) ---

                        const amountSat = Number(this.getRecoveryBetAmountSat());
                        if (!Number.isFinite(amountSat) || amountSat <= 0) {
                            try {
                                this.logger.log('⚠️ computed follow-up stake invalid; skipping follow-up', 'warning');
                            } catch (_) {
                            }
                            try {
                                if (typeof this.addSkip === 'function') this.addSkip('invalid-followup-stake', 'RECOVERY');
                            } catch (e) {
                            }
                            try {
                                this.updatePanel(this.betting.hasActiveBet(), 'RECOVERY');
                        ...
                        // Recovery multiplier must come from config/resolver (no hardcoded fallback)
                        const recMultFinal = Number(this.getCanonicalRecoveryMultiplier());
                        // Use central validated helper so invalid (zero) stakes abort locally, and pending EV clears are handled.
                        ...
                        if (placed) {
                            // IMPORTANT: only update canonical counters here if this placement did NOT
                            // consume an authoritative (pre-reserved) slot. Authoritative reservations
                            // are already consumed before the place attempt (to reserve the slot).
                            // If the slot was reserved authoritatively we must NOT subtract again here.
                            try {
                                const isAuthoritativeSlot = !!(
                                    this._authoritativeFollowup &&
                                    this._authoritativeFollowup.batchId &&
                                    slot &&
                                    slot.batchId &&
                                    String(slot.batchId) === String(this._authoritativeFollowup.batchId)
                                );

                                // For non-authoritative slots (scheduler path), decrement the canonical pending slots.
                                if (!isAuthoritativeSlot) {
                                    if (this.pendingFollowUpsSchedule && typeof this.pendingFollowUpsSchedule.slots !== 'undefined') {
                                        this.pendingFollowUpsSchedule.slots = Math.max(0, Number(this.pendingFollowUpsSchedule.slots) - 1);
                                    }

                                    // ALSO decrement authoritative remaining counter if present (keep canonical state in sync).
                                    // Note: Only decrement authoritative.remaining here for the rare case where the scheduler
                                    // supplied the slot (non-authoritative), but an authoritative object still exists in state
                                    // (e.g., scheduler and authoritative can coexist). This keeps counters consistent.
                                    try {
                                        if (this._authoritativeFollowup && Number.isFinite(Number(this._authoritativeFollowup.remaining))) {
                                            this._authoritativeFollowup.remaining = Math.max(0, Number(this._authoritativeFollowup.remaining) - 1);
                                            if (Number(this._authoritativeFollowup.remaining) <= 0) {
                                                try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ Authoritative follow-ups exhausted — clearing authoritative lock', 'info'); } catch(_) {}
                                                this._authoritativeFollowup.active = false;
                                                this._authoritativeFollowup.remaining = 0;
                                                try { if (this.pendingFollowUpsSchedule) this.pendingFollowUpsSchedule = null; } catch(_) {}
                                            }
                                        }
                                    } catch (_) { /* swallow */ }
                                } else {
                                    // Slot was previously reserved authoritatively — do not decrement authoritative.remaining here
                                    // because it was already decremented at reservation time.
                                    try {
                                        if (this.logger && typeof this.logger.log === 'function') {
                                            this.logger.log('Debug: placed authoritative follow-up slot — canonical counters were reserved earlier', 'debug');
                                        }
                                    } catch (_) { }
                                }
                            } catch (_) { /* swallow non-fatal */ }
                            // Sync engine state / UI as before
                            this._syncActiveBetFromEngine && this._syncActiveBetFromEngine();

                            try {

                                if (typeof this.updatePanel === 'function') this.updatePanel(true, 'RECOVERY-FOLLOWUP');

                            } catch (e) {

                            }
                            try {
                                this.logger.log(`🔁 Follow-up recovery bet placed: ${Math.round(amountSat / 100)} bits x${recMultFinal} (batch=${slot.batchId})`, 'bet');

                            } catch (_) {

                            }

                            // --- If configured, pause EV scheduling after N follow-ups (normal section keys) ---
                            try {
                                // only act if both config.normal.followUpPauseAfter and config.normal.followUpPauseRounds exist and are numeric
                                if (this.config && this.config.normal && Number.isFinite(Number(this.config.normal.followUpPauseAfter)) && Number.isFinite(Number(this.config.normal.followUpPauseRounds))) {

                                    const pauseAfter = Number(this.config.normal.followUpPauseAfter);
                                    const pauseRounds = Number(this.config.normal.followUpPauseRounds);

                                    // Compute followDepth defensively from available scheduler state:
                                    // prefer followCurrent (the scheduler's current slot), otherwise use pendingFollowUpsSchedule if present.
                                    let followDepthLocal = 0;
                                    try {
                                        if (typeof followCurrent !== 'undefined' && followCurrent && typeof followCurrent.total !== 'undefined' && typeof followCurrent.remaining !== 'undefined') {

                                            followDepthLocal = Number(followCurrent.total || 0) - Number(followCurrent.remaining || 0);

                                        } else if (this.pendingFollowUpsSchedule) {
                                            // pendingFollowUpsSchedule may use slots/remaining or total/remaining depending on code path

                                            const total = Number(this.pendingFollowUpsSchedule.total || this.pendingFollowUpsSchedule.slots || 0);

                                            const remaining = Number((typeof this.pendingFollowUpsSchedule.remaining !== 'undefined') ? this.pendingFollowUpsSchedule.remaining : (this.pendingFollowUpsSchedule.slots || 0));
                                            followDepthLocal = Math.max(0, total - remaining);
                                    ...
                                    if (Number.isFinite(followDepthLocal) && followDepthLocal >= pauseAfter) {

                                        if (this.bettingGate) {

                                            this.bettingGate._denyRounds = Math.max(this.bettingGate._denyRounds || 0, pauseRounds);

                                            if (typeof this.bettingGate.closeGate === 'function') this.bettingGate.closeGate('FOLLOWUP-PAUSE');
                            ...
                            // --- end followup pause hook ---
                            // If we've exhausted scheduled slots, clear scheduler and close gate to wait for EV
                            try {
                                if (Number(this.pendingFollowUpsSchedule.slots) <= 0) {
                                    try {

                                        this.logger.log('ℹ️ Follow-up sequence exhausted — clearing scheduler and closing gate, awaiting EV', 'info');
                                        ...
                                        if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                                            try {
                                                if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                                    ...
                                                    try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve followUpScheduler due to active authoritative follow-ups (exhaustion check)', 'debug'); } catch(_) {}
                                                }
                                            } catch (_) {}
                                        }
                                    } catch (e) {}
                                    ...
                                    try {
                                        if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                            ...
                                            try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve pendingFollowUpsSchedule due to active authoritative follow-ups (exhaustion check)', 'debug'); } catch(_) {}
                                        }
                                    } catch (e) {}
                                    try {
                                        if (this.bettingGate && typeof this.bettingGate.closeGate === 'function') this.bettingGate.closeGate('FOLLOWUP-EXHAUSTED');
                                        // ++ Impose a pause AND clear the stale pending request after the follow-up sequence is exhausted ++
                                        // 1. Clear the stale pendingEvRequest (which still holds the old follow-up data)
                                        this.clearPendingEvRequest();

                                        // 2. We use postRecoveryPauseRounds (default 2) to force a wait.
                                        const postPause = (this.config && this.config.normal && Number.isFinite(Number(this.config.normal.postRecoveryPauseRounds)))
                                            ? Number(this.config.normal.postRecoveryPauseRounds)
                                            : 2; // Fallback to 2 rounds

                                        if (this.bettingGate) {
                                            // 3. Set denyRounds to enforce the pause
                                            this.bettingGate._denyRounds = Math.max(this.bettingGate._denyRounds || 0, postPause);
                                            if (typeof this.bettingGate.closeGate === 'function') {
                                                this.bettingGate.closeGate('FOLLOWUP-EXHAUSTED-PAUSE');
                                            }
                                            this.logger.log(`ℹ️ Post-follow-up pause: clearing stale request and denying bets for ${postPause} rounds.`, 'info');
                                        }
                                    } catch (e) {
                                    }
                                }
                            ...
                            try {
                                this.logger.log('❌ Failed to place follow-up recovery bet (engine failure).', 'error');
                            } catch (_) {
                            }
                        }
                        return;
                    ...
                    try {
                        this.logger.log(`⚠️ scheduled follow-up handling error: ${e && e.message}`, 'warning');
                    } catch (_) {
                    }
                }
            ...
            engine.on('game_crash', (data) => {
                    ...
                    if (this.isBunkerMode) {
                        ...
                        if (crashValForBunker <= bunkerConfig.toxicThreshold) {
                            ...
                            if (trendSlope > 0.05) {
                                ...
                                this.bunkerSafeStreak = 0;
                                this.clearPendingEvRequest();
                                if (this.followUpScheduler) this.followUpScheduler.clear();
                ...
                // --- If a winning bet was EV-scheduled, cancel any pending follow-ups immediately.
                try {
                    ...
                    if (!!betResult.isWin && _source === 'EV') {
                        try {
                            if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                                try {
                                    // EV wins normally cancel follow-ups — but do NOT clear active authoritative follow-ups mid-flight.
                                    if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                        this.followUpScheduler.clear();
                                    } else {
                                        try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserved followUpScheduler despite EV win due to active authoritative follow-ups', 'debug'); } catch(_) {}
                                    }
                                } catch(_) {}
                            }
                        } catch (e) {}
                        ...
                        try {
                            if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                ...
                                try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserved pendingFollowUpsSchedule despite EV win due to active authoritative follow-ups', 'debug'); } catch(_) {}
                            }
                        } catch (e) {}

                        try {
                            this.logger.log('ℹ️ EV win detected — cleared any pending follow-up schedule (EV wins normally cancel follow-ups).', 'info');
                        } catch (e4) {}
                        try {
                            this.logger.log('ℹ️ EV win detected — cleared any pending follow-up schedule (EV wins never trigger follow-ups).', 'info');
                        } catch (e4) {
                        }
                    }
                ...
                try {
                            ...
                            if (recCfg && recCfg.enableRecovery === false) {
                                ...
                                if (typeof this.enterRecovery === 'function') {
                                    ...
                                    try {
                                        if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                                            try {
                                                // Only clear the scheduler when there is no active authoritative follow-up
                                                if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                                    this.followUpScheduler.clear();
                                                } else {
                                                    try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve followUpScheduler due to active authoritative follow-ups', 'debug'); } catch(_) {}
                                                }
                                            } catch (_) { /* swallow non-fatal */ }
                                        }
                                    } catch (e) { }
                                    ...
                                    try {
                                        // Do not clear an active authoritative-followup schedule (let authoritative flow exhaust it).
                                        if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                            this.pendingFollowUpsSchedule = null;
                                        } else {
                                            try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve pendingFollowUpsSchedule due to active authoritative follow-ups', 'debug'); } catch(_) {}
                                        }
                                    } catch (e) { }
                                } else {
                    ...
                    // RECOVERY bets
                        ...
                        if (!betResult.isWin) {
                            ...
                            // --- Schedule follow-ups ONLY when this was an EV-scheduled RECOVERY bet ---
                            // +++ normalize bet "source" detection so follow-ups trigger reliably +++
                            // Defensive: only accept betResult.meta if it's really an object
                            const meta = (betResult && betResult.meta && typeof betResult.meta === 'object' && betResult.meta !== null) ? betResult.meta : {};

                            // Guarded source extraction: prefer meta values if present, otherwise fall back to betResult.source

                            const source = String(((meta && (meta.source || meta.scheduledBy)) || betResult.source || '')).toUpperCase();

                            ;
                            // NOTE: followCfg must be mutable here because later code reassigns it
                            // (the previous const caused "Assignment to constant variable.").
                            let followCfg = (this.config && this.config.recovery && this.config.recovery.followUp) ? this.config.recovery.followUp : null;
                            const maxCount = (followCfg && Number.isFinite(Number(followCfg.maxCount))) ? Number(followCfg.maxCount) : 0;

                            // robustly detect EV-scheduled RECOVERY loss:
                            // - engine-provided 'EV' source OR
                            // - meta.batchId matching the last locally recorded ev batch id OR
                            // - a leftover local flag (this._lastPlacedWasEvRecovery) as a fallback
                            // --- robust EV-scheduled detection (covers engines that strip meta) ---
                            let isEvScheduledRecovery = false;
                            try {
                                // 1) explicit marker from engine/result
                                if (source === 'EV') {
                                    isEvScheduledRecovery = true;
                                }
                                // 2) exact batchId match to the last recorded EV batch (set when we placed the EV bet)
                                else if (meta && meta.batchId && this._lastEvRecoveryBatch && String(meta.batchId) === String(this._lastEvRecoveryBatch)) {
                                    isEvScheduledRecovery = true;
                                }
                                // 3) sentinel set when we placed an EV recovery (fallback)
                                else if (!!this._lastPlacedWasEvRecovery && String(source || '') === '') {
                                    isEvScheduledRecovery = true;
                                }
                                // 4) pendingFollowUpsSchedule explicitly recorded the triggering batchId — match it
                                else if (this.pendingFollowUpsSchedule && this.pendingFollowUpsSchedule.triggeringEvBatchId && meta && meta.batchId && String(this.pendingFollowUpsSchedule.triggeringEvBatchId) === String(meta.batchId)) {
                                    isEvScheduledRecovery = true;
                                }
                                // 5) if a pendingEvRequest exists and its batchId matches the meta.batchId (rare/safety)
                                else if (this.pendingEvRequest && this.pendingEvRequest.batchId && meta && meta.batchId && String(this.pendingEvRequest.batchId) === String(meta.batchId)) {
                                    isEvScheduledRecovery = true;
                                }
                                //     // 6) last resort: we recently created a pendingFollowUpsSchedule and we are in RECOVERY mode;
                                // //    accept this as EV-scheduled if no other marker present (helps when engine strips meta).
                                // else if (!isEvScheduledRecovery && this.pendingFollowUpsSchedule && this.pendingFollowUpsSchedule.triggeringEvBatchId && modeAtPlace === 'RECOVERY') {
                                //     isEvScheduledRecovery = true;
                                // }
                            } catch (err) {
                                // conservative fallback: consider only explicit source === 'EV'
                                isEvScheduledRecovery = (source === 'EV');
                            }
                            // --- end robust detection ---

                            // --- B: EV cooldown on EV-placed recovery loss (tiny, localized) ---
                            try {
                                if (isEvScheduledRecovery) {
                                    const cd = (this.config && this.config.recovery && this.config.recovery.EVsettings && Number.isFinite(Number(this.config.recovery.EVsettings.recoveryCooldownRounds)))
                                        ? Number(this.config.recovery.EVsettings.recoveryCooldownRounds) : 2;
                                    if (this.bettingGate) {
                                        this.bettingGate._denyRounds = Math.max(this.bettingGate._denyRounds || 0, cd);
                                        if (typeof this.bettingGate.closeGate === 'function') this.bettingGate.closeGate('EV-COOLDOWN');
                                    }
                                    // clear the recorded last EV value so delta gating doesn't incorrectly block next valid EV wins
                                    try {
                                        this._lastEvPlacedValue = null;
                                    } catch (e) {
                                    }
                            ...
                            // --- C: follow-up scheduling moved to the unified scheduling block below ---
                            // (we no longer schedule here prematurely; the unified block below
                            // performs the required proactive/joint checks before scheduling)

                            // Schedule followups only when we can reliably detect EV-scheduling and followups are enabled
                            // Only schedule follow-ups if:
                            //  - we detected the loss as EV-scheduled (isEvScheduledRecovery)
                            //  - followCfg allows follow-ups (maxCount > 0)
                            //  - followUpScheduler is available
                            //  - AND proactiveFollowUpsEnabled is true (recovery root)
                            //  - AND if recoveryRequireJointEVAndEnsemble is true, the last unified decision must also meet min confidence & EV vote
                            // Defensive: read recovery config from either this.config or this.bot.config (some call-sites use different 'this' shapes)
                            const recRoot = (this && this.config && this.config.recovery)
                                ? this.config.recovery
                                : ((this && this.bot && this.bot.config && this.bot.config.recovery) ? this.bot.config.recovery : {});

                            // follow-up sub-config (try both places)
                            const fuCfg = (recRoot && recRoot.followUp)
                                ? recRoot.followUp
                                : ((this && this.bot && this.bot.config && this.bot.config.recovery && this.bot.config.recovery.followUp)
                                    ? this.bot.config.recovery.followUp
                                    : null);

                            const proactive = !!(recRoot && recRoot.proactiveFollowUpsEnabled);
                            const requireJoint = !!(recRoot && recRoot.recoveryRequireJointEVAndEnsemble);
                            const minConf = (recRoot && recRoot.EVsettings && Number.isFinite(Number(recRoot.EVsettings.recoveryMinConfidenceForFollowup)))
                                ? Number(recRoot.EVsettings.recoveryMinConfidenceForFollowup)
                                : 0.70;

                            // --- Strict followUp control (config.recovery.followUp is authoritative) ---
                            // Behavior: WHEN an EV-scheduled RECOVERY bet loses AND followUp.enabled === true:
                            //  - schedule exactly followCfg.maxCount follow-up slots
                            //  - make them immediate (waitBetweenRounds = 0)
                            //  - mark isForced=true so consumer will bypass gates/safety
                            // This forces follow-ups to be scheduled and subsequently placed unconditionally
                            // (subject only to the existence of the scheduler object).
                            // --- Authoritative follow-up scheduling (config.recovery.followUp is authoritative) ---
                            followCfg = (this.config && this.config.recovery && this.config.recovery.followUp) ? this.config.recovery.followUp : null;
                            const enabled = !!(followCfg && followCfg.enabled);
                            const desiredCount = (enabled && Number.isFinite(Number(followCfg.maxCount))) ? Math.max(0, Math.floor(Number(followCfg.maxCount))) : 0;

                            if (!isEvScheduledRecovery) {
                                if (this.logger && typeof this.logger.log === 'function') this.logger.log('No follow-ups scheduled: not an EV-scheduled RECOVERY loss', 'info');
                            } else if (!enabled || desiredCount <= 0) {
                                if (this.logger && typeof this.logger.log === 'function') this.logger.log('No follow-ups scheduled: follow-ups disabled or maxCount<=0 in config.recovery.followUp', 'info');
                            } else {
                                const batchId = `followup-${Date.now()}`;
                                const waitBetweenRounds = 0; // immediate next-round attempts
                                const isForcedSchedule = true; // mark forced so consumer treats as authoritative

                                try {
                                    // compute expiry buffer (configurable)
                                    const nowRounds = Number((this.bot && Number.isFinite(this.bot.rounds)) ? this.bot.rounds : (Number.isFinite(this.rounds) ? this.rounds : 0));
                                    const bufferRounds = Number.isFinite(Number(followCfg.expireBufferRounds)) ? Math.max(0, Math.floor(Number(followCfg.expireBufferRounds))) : 2;
                                    ...
                                    // The placement logic requires this.pendingEvRequest to exist (with amountSat and multiplier).
                                    // Build it from the just-failed EV bet result when available (fallback sensibly).
                                    try {
                                        // Normalise amountSat (prefer betResult.betAmountSat, else betAmountBits*100 fallback)
                                        const amountSatLocal = (typeof betResult !== 'undefined' && betResult)
                                            ? (Number(betResult.betAmountSat || 0) || (Number(betResult.betAmountBits || 0) * 100))
                                            : 0;

                                        // Normalise multiplier (try betResult.multiplier, then canonical recovery getter)
                                        let multiplierLocal = (typeof betResult !== 'undefined' && betResult && Number(betResult.multiplier))
                                            ? Number(betResult.multiplier)
                                            : Number(this.getCanonicalRecoveryMultiplier() || 2.0);
                                        if (!Number.isFinite(amountSatLocal) || amountSatLocal <= 0) {
                                            // if amountSat is not available, fall back to a safe canonical stake (resolveCanonicalStakeSat)
                                            try { this.pendingEvRequest = { mode: 'RECOVERY', amountSat: this.resolveCanonicalStakeSat(null, null, 'RECOVERY'), multiplier: multiplierLocal, meta: { source: 'FOLLOWUP', batchId: String(batchId), createdAt: Date.now() } }; } catch(_) {}
                                        } else {
                                            this.pendingEvRequest = {
                                                mode: 'RECOVERY',
                                                amountSat: amountSatLocal,
                                                multiplier: multiplierLocal,
                                                meta: { source: 'FOLLOWUP', batchId: String(batchId), createdAt: Date.now() }
                                            };
                                        }

                                        // Keep a sentinel for logs/tracking: record last followup batch reference used
                                        try { this._lastFollowupPendingBatch = String(batchId); } catch(_) {}
                                    } catch (_) { /* swallow — non-fatal */ }

                                    // Ensure pendingFollowUpsSchedule is created and matches authoritative batchId
                                    try {
                                        const pf = {
                                            ...
                                            waitBetweenRounds,
                                            isForced: isForcedSchedule,
                                            triggeringEvBatchId: (meta && meta.batchId) ? String(meta.batchId) : (this._lastEvRecoveryBatch ? String(this._lastEvRecoveryBatch) : null)
                                        };
                                        // set canonical object (overwrite any stale)
                                        this.pendingFollowUpsSchedule = pf;
                                    } catch (_) { /* swallow */ }

                                    // schedule with followUpScheduler if available
                                    try {
                                        if (this.followUpScheduler && typeof this.followUpScheduler.scheduleFollowups === 'function') {
                                            this.followUpScheduler.scheduleFollowups({ batchId, count: desiredCount, isForced: isForcedSchedule, waitBetweenRounds });
                                        }
                                    } catch (e) { /* non-fatal */ }

                                    // persist canonical pending object
                                    this.pendingFollowUpsSchedule = {
                                        batchId,
                                        createdAt: Date.now(),
                                        slots: desiredCount,
                                    ...
                                    try { if (this.followupResultTracker) this.followupResultTracker.lastFollowIndex = 0; } catch(_) {}

                                    // Ensure the followUpScheduler has the canonical current batch entry immediately.
                                    // This guarantees the consumer (placement logic) sees followCurrent and will attempt
                                    // placement even if followUpScheduler.scheduleFollowups(...) returned false or the
                                    // internal scheduler was not updated for some reason.
                                    try {
                                        if (this.followUpScheduler && typeof this.followUpScheduler === 'object') {
                                            const nowRounds = Number((this.bot && Number.isFinite(this.bot.rounds)) ? this.bot.rounds : (Number.isFinite(this.rounds) ? this.rounds : 0));
                                            const nextAvailableRound = nowRounds + Math.max(0, Math.floor(Number(waitBetweenRounds) || 0));
                                            // set canonical scheduler entry (overwrite any stale)
                                            this.followUpScheduler.current = {
                                                batchId: batchId,
                                                remaining: Number(desiredCount),
                                                total: Number(desiredCount),
                                                isForced: !!isForcedSchedule,
                                                waitBetweenRounds: Number(waitBetweenRounds),
                                                nextAvailableRound
                                            };
                                        }
                                    } catch (_) { /* swallow non-fatal */ }

                                    // IMPORTANT: do NOT clear this._lastPlacedWasEvRecovery or this._lastEvRecoveryBatch here.
                                    // These sentinel flags are still useful for safe reconstruction and matching by the
                                    // consumer; clearing them too early can lead the placement code to skip follow-ups.

                                    if (this.logger && typeof this.logger.log === 'function') {
                                        this.logger.log(`Follow-ups scheduled (AUTHORITATIVE config.recovery.followUp) after EV RECOVERY loss (batch=${batchId}, slots=${desiredCount})`, 'info');
                                    }
                                } catch (e) {
                                    if (this.logger && typeof this.logger.log === 'function') this.logger.log(`⚠️ follow-up authoritative scheduling error: ${e && e.message}`, 'warning');
                                }
                                try {
                                    // CRITICAL: Clear the EV-loss flags immediately after scheduling follow-ups
                                    // to prevent follow-up losses from being misidentified as new EV losses.
                                    this._lastPlacedWasEvRecovery = false;
                                    this._lastEvRecoveryBatch = null;
                                    if (this.logger && typeof this.logger.log === 'function') {
                                        this.logger.log('ℹ️ Cleared EV-loss flags after scheduling follow-ups.', 'debug');
                                    }
                                } catch (e) { /* non-fatal */ }
                            }
                            // --- end authoritative follow-up scheduling ---
                            ...
                            if (pBitsRounded >= currentDebt) {
                                ...
                                // === START OF BUG FIX ===
                                // A full recovery win MUST clear all pending/authoritative follow-ups
                                // to prevent placing extra bets after the debt is clear.

                                try {
                                    if (this.pendingFollowUpsSchedule) this.pendingFollowUpsSchedule = null;
                                    this.logger.log('ℹ️ Full recovery: Clearing pendingFollowUpsSchedule.', 'debug');
                                } catch (e) {}
                                ...
                                // Clear the FollowUpScheduler's state
                                try {
                                    if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                                        this.followUpScheduler.clear();
                                    }
                                    this.logger.log('ℹ️ Full recovery: Clearing followUpScheduler.', 'debug');
                                } catch (e) {}

                                // Explicitly clear the authoritative lock
                                try {
                                    if (this._authoritativeFollowup) {
                                        this._authoritativeFollowup.active = false;
                                        this._authoritativeFollowup.remaining = 0;
                                        this.logger.log('ℹ️ Full recovery: Clearing _authoritativeFollowup lock.', 'debug');
                                    }
                                } catch (e) {}
                                ...
                                // --- CRITICAL: Partial recovery MUST cancel ALL remaining follow-ups, including authoritative reservations.
                                // Force-clear scheduler, pending schedule and authoritative lock so follow-ups do not continue.
                                try {
                                    // Clear the scheduler
                                    if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                                        try { this.followUpScheduler.clear(); } catch(_) {}
                                    }
                                    // Clear pendingFollowUpsSchedule (canonical object)
                                    try { this.pendingFollowUpsSchedule = null; } catch(_) {}
                                    // Clear authoritative followup lock if it exists
                                    try {
                                        if (this._authoritativeFollowup && this._authoritativeFollowup.active) {
                                            this._authoritativeFollowup.active = false;
                                            this._authoritativeFollowup.remaining = 0;
                                            // keep batchId for logs but zero remaining
                                        }
                                    } catch(_) {}
                                ...
                                // No confirmed stake but win reported: keep conservative behaviour by not scheduling follow-ups automatically.
                                try {
                                    // Do not clear an active authoritative-followup schedule (let authoritative flow exhaust it).
                                    if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                                        this.pendingFollowUpsSchedule = null;
    ...
    enterRecovery(initialDebtBits) {
        ...
        // This uses the ConfigManager OOP method and writes into
        // this.config.recovery.debtSliceAmount / debtSliceThreeAmount exactly once.
        try {
            if (this.config && typeof this.config.computeAndSetDebtSliceValues === 'function') {
                // pass the authoritative initial debt for this recovery session
                this.config.computeAndSetDebtSliceValues(this.debtBits);
            }
            // Force an initial sync to set Level 1 correctly based on the new ladder
            this.syncRecoveryLevel();
        } catch (e) { /* swallow silently */
        }

        // Important: close gate immediately and clear pending EV requests / follow-up schedules
        try {
        ...
        // Clear any stale pending EV requests so RECOVERY starts clean. EV module must re-schedule when regime approves.
        try {
            this.clearPendingEvRequest();
        } catch (e) {
        }
            ...
            if (this.followUpScheduler && typeof this.followUpScheduler.clear === 'function') {
                try {
                    if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                        ...
                        try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve followUpScheduler due to active authoritative follow-ups (enterRecovery)', 'debug'); } catch(_) {}
                    }
                } catch (_) {}
            }
        } catch (e) {}
        ...
        try {
            if (!(this._authoritativeFollowup && this._authoritativeFollowup.active && Number(this._authoritativeFollowup.remaining || 0) > 0)) {
                ...
                try { if (this.logger && typeof this.logger.log === 'function') this.logger.log('ℹ️ preserve pendingFollowUpsSchedule due to active authoritative follow-ups (enterRecovery)', 'debug'); } catch(_) {}
            }
        } catch (e) {}

        // set marker to indicate we're awaiting explicit EV/regime approval to place the first RECOVERY bet
        this.awaitingRecovery = true;

        try {
            this.logger.log(`🔻 Entered RECOVERY. Debt: ${this.debtBits} bits — gate closed, pending EV/followups cleared.`, 'warning');
        } catch (e) {
        }
    }
