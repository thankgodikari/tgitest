...
/* ===== PANEL / WATCHDOG CONFIG (no hard-coded logic elsewhere) ===== */
const PANEL_CONFIG = {
    POLL_INTERVAL_MS: 1000,                 // watch-dog poll every 1s
    MAX_REASONABLE_MULTIPLIER: 100.0,      // detect runaway counters above this
    PROVISIONAL_TIMEOUT_MS: 3000,          // fallback rollback if no confirmation arrives
    UI_DEFAULT_COLOR: '#cccccc',           // default rolling counter color (grey/white)
    UI_WIN_COLOR: '#00c853',               // green for provisional/confirmed win
    UI_LOSS_COLOR: '#e53935',              // red for loss
    SERVER_STATE_ENDPOINT: '/api/game/state', // placeholder if you have server endpoint
...
class StatsTracker {
    ...
    /**
     * Create an atomic snapshot of *all* internal state and displayed panel stats.
     * This snapshot will be used for provisional commits and rollbacks.
     */
    createSnapshot() {
        // capture internal stats object (deep copy)
        const statsCopy = JSON.parse(JSON.stringify(this.stats || {}));
        // capture crash history (shallow copy)
        const crashHistoryCopy = (Array.isArray(this.crashHistory)) ? this.crashHistory.slice() : [];
        // capture any external runtime values the panel might use (if present)
        const runtimeSnapshot = {
            timestamp: Date.now(),
            totalBets: statsCopy.totalBets || 0,
            totalWins: statsCopy.totalWins || 0,
            totalLosses: statsCopy.totalLosses || 0,
            totalProfit: statsCopy.totalProfit || 0,
            lastPLBits: statsCopy.lastPLBits,
            lastMultiplier: statsCopy.lastMultiplier,
            // Placeholders — if your main runtime stores debt/recovery info globally,
            // ensure you populate these from the canonical source before calling.
            debtBits: (this.debtBits !== undefined) ? this.debtBits : null,
            recoveryLevel: (this.recoveryLevel !== undefined) ? this.recoveryLevel : null,
            mode: (this.mode !== undefined) ? this.mode : null
        };
        return {
            stats: statsCopy,
            crashHistory: crashHistoryCopy,
            runtime: runtimeSnapshot
        };
    }

    /**
     * Restore a snapshot previously created by createSnapshot().
     * Completely replaces in-memory stats & crashHistory so UI can be reverted.
     */
    restoreSnapshot(snapshot) {
        if (!snapshot || typeof snapshot !== 'object') return false;
        try {
            this.stats = JSON.parse(JSON.stringify(snapshot.stats || {}));
            this.crashHistory = (Array.isArray(snapshot.crashHistory)) ? snapshot.crashHistory.slice() : [];
            // restore runtime-level keys if present
            const rt = snapshot.runtime || {};
            if (rt.debtBits !== undefined) this.debtBits = rt.debtBits;
            if (rt.recoveryLevel !== undefined) this.recoveryLevel = rt.recoveryLevel;
            if (rt.mode !== undefined) this.mode = rt.mode;
            // lastPL and multiplier restored by stats object
            this.logger.log(' Snapshot restored at ' + new Date().toISOString(), 'warning');
            return true;
        } catch (e) {
            this.logger.log(' Snapshot restore failed: ' + String(e), 'error');
            return false;
        }
    }
...
class CrashBot {
    ...
    processEarlyWin() {
        ...
        // 2. Execute Strategy Logic (Clears Debt, Updates P/L)
        // We pass 'true' as the 3rd arg to indicate this is a PROVISIONAL win
        this.handleStrategyState(result, false, true);

        // 3. Mark bet as processed so GAME_ENDED knows we thought we won
    ...
    rollbackEarlyWin(crashValue) {
        ...
        // 1. Revert Balance (Take back the profit we gave early)
        // Logic: We added profit, now we must subtract it AND the stake (since it's a loss)
        // However, handleStrategyState logic is complex. Easier to force a recalculation.

        // A. Restore Debt (If we cleared it)
        if (this.activeBet.modeAtPlace === 'RECOVERY') {
            // We subtracted profit from debt. Add it back.
            const assumedProfit = this.activeBet.stakeBits * (this.activeBet.targetMultiplier - 1);
            this.debtBits += assumedProfit;

            // If we switched to NORMAL, switch back to RECOVERY
            if (this.mode === 'NORMAL') {
                this.mode = 'RECOVERY';
                this.recoveryLevel = 1; // Or restore previous level if stored
            }
        }

        // B. Update Stats Tracker (Remove the fake win, Add the real loss)
        // Note: This is complex in the stats tracker, simpler to just log the loss now.
        // The stats tracker 'addBetResult' is called in handleStrategyState.
        // We need to call it with isWin=False now.
        ...
        const lossResult = {
            ...
            betAmountBits: this.activeBet.stakeBits,
            profitBits: -this.activeBet.stakeBits,
            mode: this.activeBet.modeAtPlace,
            target: this.activeBet.targetMultiplier
        };

        // Process as a standard loss
        this.handleStrategyState(lossResult, false, false);

        // Force Panel Update to RED
        this.updatePanel(false, 'ROLLBACK_LOSS', -this.activeBet.stakeBits, true);
    }
...
/* ===== SnapshotManager: handle provisional snapshots, confirm & rollback ===== */
class SnapshotManager {
    constructor(statsTracker, adapter, logger) {
        this.statsTracker = statsTracker;
        this.adapter = adapter;
        this.logger = logger;
        this.currentProvisional = null; // { snapshot, appliedAt, target }
    }

    applyProvisional(targetMultiplier, provisionalResult) {
        // Create a full snapshot of current state (all counters + runtime)
        const snap = this.statsTracker.createSnapshot();
        this.currentProvisional = {
            snapshot: snap,
            appliedAt: Date.now(),
            target: Number(targetMultiplier),
            provisionalResult: provisionalResult || {}
        };
        this.logger.log(` Provisional snapshot applied for target ${targetMultiplier}x`, 'info');
        return this.currentProvisional;
    }

    confirmProvisional() {
        if (!this.currentProvisional) return false;
        this.logger.log(` Provisional confirmed for target ${this.currentProvisional.target}x`, 'success');
        this.currentProvisional = null;
        return true;
    }

    rollbackProvisional() {
        if (!this.currentProvisional) return false;
        this.logger.log(` Provisional rollback for target ${this.currentProvisional.target}x`, 'warning');
        this.statsTracker.restoreSnapshot(this.currentProvisional.snapshot);
        this.currentProvisional = null;
        return true;
    }
}

/* ===== PanelVisuals helpers (DOM reconciliation + coloring rules) ===== */
class PanelVisuals {
    constructor() {
        // Query common panel elements (only if they exist)
        this.el = {
            rollingCounter: document.getElementById('rollingCounter'),
            ...
            lastPL: document.getElementById('lastPL'),
    ...
    setRollingColor(color) { if (!this.el.rollingCounter) return; this.el.rollingCounter.style.color = color; }
    setTargetColor(color) { if (!this.el.targetMultiplier) return; this.el.targetMultiplier.style.color = color; }
    setLastCrashColor(color) { if (!this.el.lastCrash) return; this.el.lastCrash.style.color = color; }
    updateStatsFrom(stats) {
        if (!stats) return;
        if (this.el.debt) this.el.debt.textContent = (stats.runtime && stats.runtime.debtBits !== undefined) ? stats.runtime.debtBits : (stats.totalProfit || 0).toFixed(2);
        if (this.el.pl) this.el.pl.textContent = (stats.totalProfit || 0).toFixed(2);
        if (this.el.winCounter) this.el.winCounter.textContent = (stats.totalWins || 0);
        if (this.el.betCounter) this.el.betCounter.textContent = (stats.totalBets || 0);
        if (this.el.lastPL) this.el.lastPL.textContent = (stats.lastPLBits !== null && stats.lastPLBits !== undefined) ? stats.lastPLBits.toFixed(2) : '-';
    }
    appendLog(msg, type = 'info') {
        if (!this.el.panelLog) return;
        const row = document.createElement('div');
        row.textContent = `[${new Date().toLocaleTimeString()}] ${msg}`;
        this.el.panelLog.prepend(row);
    }
    setWatchdogStatus(txt) { if (!this.el.watchdogStatus) return; this.el.watchdogStatus.textContent = txt; }
}

/* ===== PanelWatchdog: polls server every 1s and enforces sync + infinite-counter fix ===== */
class PanelWatchdog {
    constructor(adapter, statsTracker, snapshotManager, logger, visuals) {
        this.adapter = adapter;
        this.stats = statsTracker;
        this.snapshotManager = snapshotManager;
        this.logger = logger;
        this.visuals = visuals;
        this._interval = PANEL_CONFIG.POLL_INTERVAL_MS || 1000;
        this._timer = null;
        this._lastServerGameId = null;
        this._lastServerCrash = null;
    ...
    start() {
        ...
        this.visuals.setWatchdogStatus('WATCHDOG: running');
        this.logger.log('PanelWatchdog started', 'info');
    }
    ...
    stop() {
        ...
        this._timer = null;
        this._isRunning = false;
        this.visuals.setWatchdogStatus('WATCHDOG: stopped');
    }

    async tick() {
        try {
            // The adapter may expose a lightweight API to fetch authoritative game state
            // If not, the adapter should be extended. Here we attempt to call a common getState()
            const serverState = (typeof this.adapter.getServerState === 'function') ? await this.adapter.getServerState() : null;

            // If no serverState available, we still refresh GUI from in-memory stats to keep panel alive
            if (!serverState) {
                this.visuals.updateStatsFrom(this.stats.stats);
                return;
            }

            // Reconcile rolling value vs server
            const gameStatus = serverState.status || serverState.gameStatus || 'UNKNOWN';
            const serverCrash = serverState.lastCrashValue || serverState.lastCrash || null;
            const serverRolling = serverState.rolling || serverState.currentMultiplier || null;
            const serverGameId = serverState.gameId || serverState.id || null;

            // Infinite counter guard: if local is running but server says ENDED, force resync
            if (gameStatus === 'ENDED' && serverGameId && this._lastServerGameId && serverGameId === this._lastServerGameId) {
                // server still says ENDED for the same game — ensure rolling reset
                this.logger.log(' Watchdog detected local running while server reports ENDED — forcing resync', 'warning');
                this.visuals.appendLog('Resync: server reported ENDED while local running', 'warning');
            }

            // Detect runaway multiplier (client bug) and resync
            if (serverRolling && serverRolling > PANEL_CONFIG.MAX_REASONABLE_MULTIPLIER) {
                this.logger.log(' Watchdog detected runaway multiplier > MAX_REASONABLE_MULTIPLIER; forcing reset', 'warning');
                this.visuals.appendLog('Runaway multiplier detected; forcing reset', 'warning');
            }

            // Update last seen game id + crash
            this._lastServerGameId = serverGameId || this._lastServerGameId;
            this._lastServerCrash = serverCrash || this._lastServerCrash;

            // Refresh UI from authoritative stats when available
            if (serverState.panelStats) {
                // panelStats is expected to be a serializable object mirroring StatsTracker.stats + runtime
                this.visuals.updateStatsFrom(serverState.panelStats);
            } else {
                this.visuals.updateStatsFrom(this.stats.stats);
            }
        } catch (err) {
            this.logger.log('Watchdog tick error: ' + String(err), 'error');
        }
    }
}

/* ===== Bootstrapping the panel helpers (if DOM exists) ===== */
try {
    // Attempt to wire-up if running in a browser UI
    if (typeof window !== 'undefined' && document) {
        const globalAdapter = (typeof unifiedEngineAdapter !== 'undefined') ? unifiedEngineAdapter : (typeof adapter !== 'undefined' ? adapter : null);
        // The script may create a ConfigManager and StatsTracker with different variable names.
        // Try to detect common names (statsTracker, stats) — fallback to creating a new StatsTracker instance.
        const detectedStats = (typeof statsTracker !== 'undefined') ? statsTracker : (typeof StatsTracker !== 'undefined' ? new StatsTracker(new ConfigManager(globalAdapter)) : null);
        const detectedLogger = (typeof logger !== 'undefined') ? logger : new Logger((typeof configManager !== 'undefined') ? configManager : new ConfigManager(globalAdapter));
        const snapshotMgr = new SnapshotManager(detectedStats, globalAdapter, detectedLogger);
        const visuals = new PanelVisuals();
        const watchdog = new PanelWatchdog(globalAdapter, detectedStats, snapshotMgr, detectedLogger, visuals);
        // Expose for debugging
        window.__PANEL = { snapshotMgr, visuals, watchdog };
        watchdog.start();
    }
} catch (e) {
    console.warn('Panel bootstrapping failed: ', e);
}
